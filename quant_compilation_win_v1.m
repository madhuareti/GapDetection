...
% dir_str = strcat("results",slsh,"quant_results",slsh,"oitra_10_13",slsh);
% dir_str = strcat("results",slsh,"quant_results",slsh,"gitra_10_14",slsh);
...
while min(size(data_strs_loop)) >= 1
    ...
    param_key_temp3 = split(param_key_temp2{1},slsh);
        if isempty(param_key_temp3) % if all elements are empty 
            ...
            if size(param_key1_temp2,1) > 3
                if strcmp(param_key1_temp2{end},'hole')
                    param_key1 = strcat(param_key1,"_",param_key1_temp2{end});
                else
    ...
    while ~isempty(cur_sample_list{samp_ct})
        ...
        comp_quant_struct(qt_ct).TP = TP_num_ref;
        comp_quant_struct(qt_ct).FP = FP_num_ref;
        comp_quant_struct(qt_ct).TN = TN_num_ref;
        comp_quant_struct(qt_ct).FN = FN_num_ref;
        comp_quant_struct(qt_ct).MCC = AlgoMCC_arr_ref;
        comp_quant_struct(qt_ct).mGIoU = giou_struct.mgiou;
        comp_quant_struct(qt_ct).ACC = AlgoAccuracy_arr_ref;
        comp_quant_struct(qt_ct).F1 = AlgoF1Score_arr_ref;
        comp_quant_struct(qt_ct).Precision = AlgoPrecision_arr_ref;
        comp_quant_struct(qt_ct).Recall = AlgoRecall_arr_ref;
