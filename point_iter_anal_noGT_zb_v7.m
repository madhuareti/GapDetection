...
num_iter = 175; % number of iterations to be performed
...
save_str_init = strcat("results",slsh,"oriented_xyz_1107",slsh);
sample_bin = false; % based on whether analyzing raw panel data or sampled data
ip_str = strcat(local_str,dir_data_str)
rect_wall_thick = 4*2.54; % 4inch thick panels
% num_vals_vec = 20;
num_vals_vec = 20;
rad_range_vec = 0.25:0.15:1.25;
chunk_size = 2500; % chunk size for data division for parallel processing
...
mat_key = '.mat';
dir_strs = dir_strs(logical(cellfun(@(x) MatchKeys(x, mat_key), {dir_strs.name}))); % paths of all directories (unique datasets)
...
ip_sample_itr = size(dir_strs,1) + 1; %this will be editted by the bash script
while ip_sample_itr > 1
	% incase bash script double writes the counter
    if ip_sample_itr > size(dir_strs,1)
	    ip_sample_itr=size(dir_strs,1)+1;
    end
    ip_sample_itr=ip_sample_itr-1;% spaces removed on purpose for sed
%     change env variable for matlab crash accomodation
    ...
    if isfolder(cur_ip_dir_str)
	    ...
	    sample_tag = replace(sample_tag_temp{2},strcat(slsh,"sample"),"");
        save_str1 = strcat(save_str_init,sample_tag,slsh);
    else
        cur_ip_dir_str = dir_strs(ip_sample_itr).folder;
        data_list = dir(fullfile(cur_ip_dir_str,'*.mat'));
        sample_tag_temp = split(cur_ip_dir_str,ip_str);
	    sample_tag = replace(sample_tag_temp{end},".mat","");
        save_str1 = strcat(save_str_init,sample_tag,slsh);
