...
dmg_data_str = strcat("damaged_points_comp_untrans_all",slsh);
data_str1 = strcat("mat_xyz_all",slsh); % data path inside upper level
save_str0 = strcat("sample_data_complete_all",slsh);
save_str00 = strcat("raw_data_complete",slsh);
save_str000 = strcat("damaged_points_complete_all",slsh);
save_str1 = strcat(local_str,data_str0,save_str0); % save str for the data samples
save_str001 = strcat(local_str,data_str0,save_str00); % save str for pca transformation analysis
save_str0001 = strcat(local_str,data_str0,save_str000); % save str for damage points transformation analysis
ip_str = strcat(local_str,data_str0,save_str00,data_str1); % data path relative to local/rdrive str
dmg_data_str_temp = strcat(local_str,data_str0,dmg_data_str);
del_keys = [".";".."];
dmg_data_strs = dir(fullfile(dmg_data_str_temp)); % extract all files in the directory
...
for itr = 1 : size(data_strs,2)
    ...
    if isfolder(cur_data_path)
        cur_dir_strs = CheckFolderSetup(cur_data_path,del_keys,slsh,'.mat');
    else
        cur_dir_strs = string(cur_data_path);
    end
    for itr2 = 1 : size(cur_dir_strs,2)
        cur_data_path = cur_dir_strs(itr2).val;
        str_tag_temp = split(cur_data_path,data_str1);
        str_tag = str_tag_temp{2};
        str_tag = replace(str_tag,".mat",""); % file name
        save_str = strcat(save_str1,str_tag,slsh); % directory for all subsamples
        load(cur_data_path);
    ...
    %                 else
    %                     % check if atleast one value is registered from current
    %                     % slice width
    %                     if slce_tol_vec(vec_ct,1) ~= slce_wid_vec(itr1)
    %                         vec_ct = vec_ct + 1;
    %                         slce_tol_vec(vec_ct,1) = slce_wid_vec(itr1);
    %                         slce_tol_vec(vec_ct,2) = tol_val_vec(itr2);
    %                     end
    ...
    %     % plot3(filter_pc(:,1),filter_pc(:,2),filter_pc(:,3),'.')
        save_dir_str1 = strcat(save_str001,slsh,"oriented_xyz_all",slsh);
        save_dir_str2 = strcat(save_str001,slsh,"transf_matrices_all",slsh);
        ...
        % transform the damaged points data
        temp_split = split(str_tag,"hole"); % check for the hole tag
        if size(temp_split,1) > 0
            str_tag_main = temp_split{1};
        else
            str_tag_main = str_tag;
        end
        cur_data_str_temp = dmg_data_strs(logical(cellfun(@(x) MatchKeys(x, str_tag_main), {dmg_data_strs.name}))); % path of current panel 
        if size(cur_data_str_temp,1) < 1
            continue;
        end
        dmg_str = strcat(cur_data_str_temp.folder,slsh,cur_data_str_temp.name);
        load(dmg_str);
        org_ip_pc = round(org_ip_pc,2);
        org_ip_pc = org_ip_pc*100;
        ip_zero_cntrd = bsxfun(@minus, org_ip_pc, -trans_matrix(1:3,4)');
        ip_zero_cntrd = round(ip_zero_cntrd,3);
        orient_pts = trans_matrix(1:3,1:3)*ip_zero_cntrd';
        orient_pts = round(orient_pts',3);
        damaged_pts = RepeatRemove3D(orient_pts);
        org_ip_pc = damaged_pts;
        if ~isfolder(save_str0001)
            mkdir(save_str0001);
        end
        trans_dmg_save_str = strcat(save_str0001,str_tag,".mat");
        save(trans_dmg_save_str,'org_ip_pc','-v7.3');
        continue;
    
        load(trans_data_save_str)
        %% 3D Slicing
        ...
        for itr1 = 1: size(slce_tol_vec,1)
            ...
            if size(cartographic_result,1) > 0
		        ...
		        disp("directory made");
                    ...
                    save_str_final = strcat(save_str,"sample_",string(slce_tol_vec(itr1,1)),...
                        "_",strrep(string(round(slce_tol_vec(itr1,2),2)),'.',''),'.mat')
                try 
	               save(save_str_final,'cartographic_result','-v7.3');
	            catch err
	               disp("Error saving the .mat file");
                end
            else
                continue;
            end
        ...
        end % end of sample generation
        try
    	    writetable(struct2table(sampling_struct),strcat(save_str,'params_',str_tag,".csv"));
        catch err
    ...
    end % end of looping over smaples for the current panel
end % end of looping over all the input panel
