...
nn_mult_vec = 4; 
ThrshMultFacArrTemp = -0.1;
ThrshMulFacArr = -0.1;
nn_mult_vec_ref = 4;
ThrshMulFac_Ref = -1.9;
% save strs
nn_mult_vec_str = string(nn_mult_vec);
thrsh_mult_vec_str = strrep(string(ThrshMultFacArrTemp),'.',"");
% nn_mult_vec = (1:2)';
...
data_str1 = strcat("sample_data_complete_1007",slsh,dir_key,slsh);
...
for ip_sample_itr = 1 : size(dir_strs,1)
    ...
    for ip_data_dir_itr = 1 : size(data_list,1)
        ...
        file_name = string(file_name_temp(1));
	...
	gt_path_str = string(strcat(local_str,dir_str,gt_str1,slsh,dir_key,'.mat'))
	if strcmp(dir_key,"")
	    dir_key = split(data_str1,slsh);
	    dir_key = string(dir_key{2});
	end
	save_str1 = strcat("results",slsh,"GITRA",slsh,"10_13",slsh,dir_key,slsh)
	save_str1 = replace(save_str1,strcat(slsh,slsh),slsh);
        save_str = strcat(local_str, save_str1,file_name,slsh);
	if isfolder(save_str) % skip analysis if a file already exists
		continue;
	end
        load(gt_path_str);
...
%                     time_per_iter(iter_itr,2) = toc; % NN extraction time
                    parfor th_itr = 1 : size(ThrshMulFac_Ref,1)
                        damaged_points_cur_ref = [];
                        undamaged_points_cur_ref = [];
                        ...
                        [damaged_points_cur_ref,rem_points,time_elapsed] = ...
                            ...
                            ThrshMulFac_Ref(th_itr),chunk_size); 
                    ...
                    best_thmul_vec(nn_mult_itr,1) = ThrshMulFac_Ref(cur_opt_idx);
