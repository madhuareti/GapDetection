...
%% Parameters
num_iter = 5; % number of iterations to be performed
chunk_size = 1500; % chunk size for data division for parallel processing
...
local_str = "/home/grads/m/mareti/matlab_ws/GapDetect/";
...
while ip_sample_itr > 5
    ...
    ip_data_dir_itr = size(data_list,1)+1
    panel_name_temp = split(data_list(1).folder,slsh);
    ...
    while ip_data_dir_itr > 1
	ip_data_dir_itr = ip_data_dir_itr - 1;
        %% load data
	...
	save_str1 = strcat("results",slsh,"GITRA",slsh,"oriented_xyz_1014",slsh,panel_name,slsh)  
...
%                     time_per_iter(iter_itr,2) = toc; % NN extraction time
	...
	OutputPoints_ref_struct_opt(ip_data_dir_itr).panel_name = panel_name;
	OutputPoints_ref_struct_opt(ip_data_dir_itr).file_name = file_name;
