...
try
  % p = parcluster('local'); % given in the bash file
...
sample_tag = "oriented_xyz";
%dir_data_str = strcat("data",slsh,"disaster_city",slsh,"sample_data_complete");
dir_data_str = strcat("data",slsh,"disaster_city",slsh,"raw_data_complete",slsh);
save_str1 = strcat("results",slsh,sample_tag,slsh);
ip_str = strcat(local_str,dir_data_str)
rect_wall_thick = 4*2.54; % 4inch thick panels
num_vals_vec = 20;
rad_range_vec = 0.15:0.15:1.25;
chunk_size = 15000; % chunk size for data division for parallel processing
...
%     change env variable for matlab crash accomodation
    ...
    for ip_data_dir_itr = 5 : size(data_list,1)
	    % change env variable for matlab crash accomodation
	    %% load data
